...
public class MemberBulletinService : IBulletinTypeListService<MemberBulletin>
{
    ...
    public async Task<MemberBulletinDetailResponse?> UpdateMemberBulletinAsync(string id, UpdateMemberBulletinRequest request, string memberId, UserRole role)
    {
        if (!ObjectId.TryParse(id, out ObjectId objectId))
        {
            return null;
        ...
        FilterDefinition<MemberBulletin> filter;
        if (role == UserRole.Administrator)
        {
            filter = Builders<MemberBulletin>.Filter.Eq("_id", objectId);
        }

        else
        {

            filter = Builders<MemberBulletin>.Filter.And(
    ...
    public async Task<bool> DeleteMemberBulletinAsync(string id, string memberId, UserRole role)
    {
        if (!ObjectId.TryParse(id, out ObjectId objectId))
        {
            return false;
