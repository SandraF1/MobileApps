...
public class MemberBulletinController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<MemberBulletinDetailResponse>>> UpdateMemberBulletin(string id, [FromBody] UpdateMemberBulletinRequest request)
    {
        ...
        {
            return Unauthorized(ApiResponse<MemberBulletinDetailResponse>.ErrorResponse("Unable to identify role"));
        }

        var response = await _memberBulletinService.UpdateMemberBulletinAsync(id, request, currentUserId, currentUserRole.Value);
        if (response == null)
        {
    ...
    public async Task<ActionResult> DeleteMemberBulletin(string id)
    {
        ...
        var result = await _memberBulletinService.DeleteMemberBulletinAsync(id, currentUserId, currentUserRole.Value);
        if (!result)
        {
